...
motif_obs <- as.data.frame(motif_pca$x) %>% 
  mutate(painting_code = rownames(motif_pca$x)) %>% 
...
artist_motif_plot <- function(artist_name) {
  motif_obs %>% 
    mutate(highlight_artist = ifelse(artist %in% artist_name, artist, NA), point_size = ifelse(is.na(highlight_artist), 2, 5)) %>% 
...
artist_motif_plot(c("Floris van Dijck", "Willem Claesz Heda"))
```
...
per_artist_dist %>% 
  ...
  ggplot(aes(x = motif_asymm, y = composition, color = artist_detail)) +
