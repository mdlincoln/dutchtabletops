...
motif_matrix <- motif_data %>% select(-painting_code)
...
Principal component analysis is another way to visualize these clusterings. As it turns out, our clusters are less than lovely.
...
# just project into the first 2 PCA axes
...
What's that, you wanted three principal components?

```{r motif_pca3d, cache=FALSE}
scatterplot3js(pca_df$PC1, pca_df$PC2, pca_df$PC3)
```
