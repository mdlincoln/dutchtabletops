...
motif_data <- sl_motifs %>% 
  ...
  inner_join(select(dt_paintings, painting_code, artist, painting_description, year), by = "painting_code") %>% 
...
```{r motif_symm_dendrogram, eval=FALSE}
n_clust <- 10
...
```{r motif_asymm_dendrogram, eval=FALSE}
# Asymetric binary
...
motif_data %>% 
  mutate(group = cutree(asymm_motif_clust, k = n_clust)) %>% 
  select(group, painting_code, painting_description, artist, year) %>% 
  datatable()
```

Principal component analysis is another way to visualize these clusterings.

```{r motif_pca, eval=FALSE}
pca <- prcomp(motif_matrix)
prin <- princomp(motif_matrix)
tpca_projection <- predict(pca, newdata = motif_matrix)

# just project into the first 2dis PCA axes
pca_df <- as.data.frame(pca_projection[,1:3]) %>% 
  mutate(cluster = cutree(symm_motif_clust, k = 6))
