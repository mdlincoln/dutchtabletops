...
model_subsets <- list(
  ...
  all_qualifications = which(dt_valid$artist_union %in% cleared_artist_union & dt_valid$artist %in% cleared_artists)
)
...
rfl <- map(model_params, function(x) run_rf(dt_valid, response = x$response, predictors = x$predictors, ntree = 500, subset = x$subsets))
...
per_artist_error <- rfl_df %>% 
  filter(response == "qualified" & subset == "all_qualifications" & predictors %in% c("motif_only", "composition_only")) %>% 
...
pae <- ggplot(per_artist_error, aes(x = composition_only, y = motif_only, color = artist)) +
  ...
  geom_point(aes(shape = qualification), size = 5) +
  theme_bw() +
...
# This is a bad way to test, because it tests the same ones it saw
...
run_prediction_error <- function(model) {
  pv <- dt_compiled %>% 
    filter(artist %in% cleared_artists) %>% 
    prep_vars("painting_code") %>% 
...
```{r known_only_test}
fctr_dt_compiled <- prep_vars(dt_compiled)
...
kunk_rf <- function(train, test, predictors) {
  ...
  randomForest(x = train_df, y = train_response, xtest = test_df, ytest = test_response, ntree = 2000, importance = TRUE, localImp = TRUE, proximity = TRUE)
}
