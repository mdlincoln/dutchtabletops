...
rfv <- rf_votes(known_only_motif) %>% 
  gather(predicted, votes, -painting_code) %>% 
  inner_join(dt_compiled, by = "painting_code") %>% 
  filter(predicted == "Pieter Claesz") %>% 
  select(votes, artist, one_of(c("LV", "N", "T1", "T", "Ch", "LV2", "Fi"))) %>% 
  gather(term, value, -votes, -artist) %>% 
  filter(value == TRUE)

ggplot(rfv, aes(x = term, y = votes, color = artist == "Pieter Claesz")) +
